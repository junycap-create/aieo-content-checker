...
const SnippetPreview: React.FC<SnippetPreviewProps> = ({ snippets }) => {
  // Rotate logos for variety in snippets (Mocking the concept that different snippets come from different engines)
  const getIcon = (idx: number) => {
    if (idx % 3 === 0) return <GoogleIcon />;
    if (idx % 3 === 1) return <PerplexityIcon />;
    return <OpenAIIcon />;
  };

  const getName = (idx: number) => {
    if (idx % 3 === 0) return 'Google AI Overview';
    if (idx % 3 === 1) return 'Perplexity';
    ...
    <div className="bg-white rounded-xl shadow-lg shadow-zinc-200/50 border border-zinc-200 p-8">
      ...
      <div className="grid gap-6">
        {snippets.map((item, idx) => (
          <div key={idx} className="bg-zinc-50 border border-zinc-200 rounded-lg overflow-hidden hover:border-zinc-400 transition-colors group">
            ...
            <div className="bg-white border-b border-zinc-100 px-4 py-3 flex items-center justify-between">
                <div className="flex items-center gap-2 shrink-0">
                    {getIcon(idx)}
                    <span className="text-xs font-bold font-mono text-zinc-600 uppercase tracking-wide">{getName(idx)}</span>
                </div>
            ...
            <div className="p-6">
                <div className="flex items-start gap-3 mb-4 pb-4 border-b border-zinc-100 border-dashed">
                    ...
                    <p className="text-lg font-bold text-zinc-900 leading-snug">{item.question}</p>
                </div>
                <div className="flex items-start gap-3">
                    <span className="text-zinc-400 font-mono font-bold text-lg min-w-[20px] shrink-0">A.</span>
                    <p className="text-base text-zinc-600 leading-relaxed">{item.answer}</p>
                </div>
            </div>
          </div>
        ))}
